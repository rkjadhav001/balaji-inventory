...
<body>
    <div id="mobile-content">
        <form id="orderForm" action="{{ route('Wholesaler.Order') }}" method="POST">
            @csrf
            <div
                style="padding: 2px 10px;width: 100%; display: flex; justify-content: space-between; align-items: center;background-image: linear-gradient(to top, #dbdbdb 0%, #FED6E3 100%);    box-shadow: rgba(149, 157, 165, 0.4) 0px 8px 24px;padding-right: 0px !important;padding-left: 0px !important;">
                <img src="https://www.balajiwafers.com/wp-content/themes/custom/img/BalajiWafers.svg" alt=""
                    style="max-height: 90px;padding: 10px;">
                <div>
                    <a class="btn btn-select-login " href="#"
                ...
                <div class="row mt-4">
                    @foreach ($products as $key => $product)
                        <div class="col-md-12 {{ $key != 0 ? 'mt-3' : '' }}">
                            <div class="card" data-box="{{ $product->box }}" data-patti="{{ $product->patti }}" data-packet="{{ $product->packet }}" data-selling_price="{{ $product->selling_price }}"
                                data-per_patti_paket="{{ $product->per_patti_piece }}">
                                <div class="card-header">
                                    <p style="color: #403092;font-weight: 500;margin: 0;text-align: left;">
                                        {{ $product->name }}</p>
                                    <div class="d-flex justify-content-between">
                                        <p style="margin: 0;"><span style="color: #c77642;font-weight: 500;"
                                                class="">Total Qty:
                                            </span>
                                            <span class="productTotalQty">0Pkt</span></p>
                                        <input type="hidden" name="products[{{ $product->id }}][productTotalQty]" class="productTotalFormQty">
                                        <p style="margin: 0;"><span style="color: #507a53;font-weight: 500;"
                                                class="">Total Cost:
                                            </span>
                                            <span class="productTotalCost">0.00/INR</span>
                                        </p>
                                        <input type="hidden" name="products[{{ $product->id }}][productTotalCost]" class="productTotalFormCost">
                                    </div>
                                </div>
                                <div class="card-body">
                                    ...
                                    <div class="row">
                                        ...
                                        @if (in_array('1', $unitTypesArray))
                                            <div class="col-{{ 12 / count($product->unit_type_names) }}">
                                                ...
                                                <input type="number" name="products[{{ $product->id }}][box]" id="box" class="form-control productBox"
                                                    value="0" placeholder="Box">
                                            </div>
                                        @endif
                                        @if (in_array('2', $unitTypesArray))
                                            <div class="col-{{ 12 / count($product->unit_type_names) }}">
                                                <label for="patti">Patti</label>
                                                <input type="number" name="products[{{ $product->id }}][patti]" id="patti" class="form-control productPatti"
                                                    value="0" placeholder="Patti">
                                            </div>
                                        @endif
                                        @if (in_array('3', $unitTypesArray))
                                            <div class="col-{{ 12 / count($product->unit_type_names) }}">
                                                <label for="packet">Packet</label>
                                                <input type="number" name="products[{{ $product->id }}][packet]" id="packet" class="form-control productPacket"
                                                    value="0" placeholder="Packet">
                                            </div>
                                        @endif
                                    </div>
                                </div>
            ...
            <footer>
                <div class="d-flex justify-content-between">
                    <div class="d-flex" style="flex-direction: column;">
                        <span style="font-weight: 600;color: #403092" class="totalProducts">Total Products : <span id="totalProducts" style="color: #fea272">0</span></span>
                        <input type="hidden" name="totalProducts" id="totalProductsHidden" value="0">
                        <span style="font-weight: 600;color: #403092" class="finalTotalAmount">Total Amount : <span id="finalTotalAmount" style="color: #fea272">0</span></span>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#add-product').on('click', function () {
                ...
                const confirmation = confirm(`You are about to place an order with ${totalProducts} products for a total of ${finalTotalAmount}. Do you want to proceed?`);
                if (confirmation) {
                    $('#orderForm').submit();
                }
            });
